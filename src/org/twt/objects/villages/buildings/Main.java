...
/** This class represents a main building with its properties and calculations
 * 
 * @author JavaBeginner
 *
 */
public class Main extends AbstractBuilding
{
    /** Construct a new main building object
     * 
     ...
     */
    public Main(String name, int maxLevel, int minLevel, int wood, int stone,
            int iron, int pop, double woodFactor, double stoneFactor,
            double ironFactor, double popFactor, double buildTime,
            double buildTimeFactor, int level1Points)
    {
        super(name, maxLevel, minLevel, wood, stone, iron, pop, woodFactor,
                stoneFactor, ironFactor, popFactor, buildTime, buildTimeFactor, level1Points);
    }
    
    /** Get the build time in seconds needed for level x with main building on level y.
     * <p> The current level of the main building is for a main building object well known (level x-1),
     * so you don't need to pass it to the method
     * @param level The level for the build time calculation
     * @return The build time in seconds
     */
    public int getBuildTimeNeeded(int level)
    {
        return super.getBuildTimeNeeded(level, (level - 1));
    }
}
